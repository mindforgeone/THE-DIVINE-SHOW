...
export default function MemberApp({ user, state, commit, currentDate, syncState, onLogOut }) {
  ...
  return <div className="min-h-screen bg-[#f2f7f6] pb-24 text-[#15333b]">
    ...
    <main className="mx-auto grid max-w-5xl gap-4 px-3 py-4 sm:px-5">
      ...
      {view === 'profile' && <MemberProfile user={user} state={state} commit={commit} />}
...
function MemberProfile({ user, state, commit }) {
  ...
  return <div className="grid gap-4"><section className="border border-[#cfe0dc] bg-white p-4 rounded-lg"><div className="flex items-center gap-3"><img src={draft.photoUrl || user.photoURL || ''} alt="" className="h-16 w-16 bg-[#dfeae7] object-cover rounded-md" /><div><div className="text-sm font-black text-[#0d735f]">Личные данные</div><h1 className="text-2xl font-black">{draft.displayName || user.displayName || 'Мой профиль'}</h1></div></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><Field label="Имя"><input value={draft.displayName || ''} onChange={(event) => setDraft({ ...draft, displayName: event.target.value })} className="field-control mt-1" /></Field><Field label="Фото: ссылка"><input value={draft.photoUrl || ''} onChange={(event) => setDraft({ ...draft, photoUrl: event.target.value })} className="field-control mt-1" /></Field><label className="flex min-h-11 cursor-pointer items-center justify-center gap-2 border border-[#b9ddd3] bg-[#eaf8f4] font-black text-[#0d735f] rounded-md sm:col-span-2"><Camera size={17} />{uploading ? 'Готовлю фото…' : 'Выбрать своё фото'}<input type="file" accept="image/*" disabled={uploading} onChange={(event) => uploadAvatar(event.target.files?.[0])} className="sr-only" /></label><Field label="Возраст"><input type="number" value={draft.age || ''} onChange={(event) => setDraft({ ...draft, age: event.target.value })} className="field-control mt-1" /></Field><Field label="Рост, см"><input type="number" value={draft.height || ''} onChange={(event) => setDraft({ ...draft, height: event.target.value })} className="field-control mt-1" /></Field><Field label="Целевой вес, кг"><input type="number" step="0.1" value={draft.targetWeight || ''} onChange={(event) => setDraft({ ...draft, targetWeight: event.target.value })} className="field-control mt-1" /></Field><Field label="Лимит калорий"><input type="number" value={draft.calorieTarget || ''} onChange={(event) => setDraft({ ...draft, calorieTarget: event.target.value })} className="field-control mt-1" /></Field></div><label className="mt-3 flex items-start gap-3 border border-[#d7e5e1] bg-[#f5faf8] p-3 rounded-md"><input type="checkbox" checked={Boolean(draft.trackActiveCalories)} onChange={(event) => setDraft({ ...draft, trackActiveCalories: event.target.checked })} className="mt-1 h-5 w-5 accent-[#0d8b71]" /><span><strong className="block text-sm">У меня есть трекер активных калорий</strong><small className="text-slate-500">Если выключено, активные калории не требуются и не влияют на закрытие дня.</small></span></label><div className="mt-3 grid gap-2 sm:grid-cols-3"><Privacy checked={draft.discoverable !== false} label="Меня можно найти" onChange={(discoverable) => setDraft({ ...draft, discoverable })} /><Privacy checked={Boolean(draft.shareProgress)} label="Делиться процентом пути" onChange={(shareProgress) => setDraft({ ...draft, shareProgress })} /><Privacy checked={Boolean(draft.shareWeight)} label="Делиться весом" onChange={(shareWeight) => setDraft({ ...draft, shareWeight })} /></div>{uploadError && <div className="mt-2 text-xs font-bold text-rose-600">{uploadError}</div>}<button type="button" onClick={saveProfile} className="mt-4 min-h-12 w-full bg-[#0d8b71] font-black text-white rounded-md">Сохранить профиль</button></section>
    <section className="border border-[#cfe0dc] bg-white p-4 rounded-lg"><div className="flex items-center gap-2"><Scale size={19} className="text-[#0d8b71]" /><h2 className="text-xl font-black">Замеры и фото прогресса</h2></div><p className="mt-2 text-sm font-semibold leading-6 text-slate-500">Вес, талия, грудь, бёдра, рука и бедро дают более полную картину, чем одна цифра на весах. Талию измеряй в одинаковых условиях, лента горизонтально, после обычного выдоха.</p><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4"><Field label="Дата"><input type="date" value={log.date} onChange={(event) => setLog({ ...log, date: event.target.value })} className="field-control mt-1" /></Field>{[['weight', 'Вес, кг'], ['waist', 'Талия, см'], ['chest', 'Грудь, см'], ['hips', 'Бёдра, см'], ['arm', 'Рука, см'], ['thigh', 'Бедро, см']].map(([id, label]) => <Field key={id} label={label}><input type="number" step="0.1" value={log[id]} onChange={(event) => setLog({ ...log, [id]: event.target.value })} className="field-control mt-1" /></Field>)}</div><Field label="Ссылка на фото"><input value={log.photoUrl} onChange={(event) => setLog({ ...log, photoUrl: event.target.value })} className="field-control mt-1" /></Field><label className="mt-2 flex min-h-11 cursor-pointer items-center justify-center gap-2 border border-[#b9ddd3] bg-[#eaf8f4] font-black text-[#0d735f] rounded-md"><Camera size={17} />{uploading ? 'Загружаю…' : 'Загрузить фото'}<input type="file" accept="image/*" disabled={uploading} onChange={(event) => upload(event.target.files?.[0])} className="sr-only" /></label>{uploadError && <div className="mt-2 text-xs font-bold text-rose-600">{uploadError}</div>}<button type="button" disabled={!Object.entries(log).some(([key, value]) => key !== 'date' && value)} onClick={() => { addLog(); setLog({ date: todayKey(), weight: '', waist: '', chest: '', hips: '', arm: '', thigh: '', photoUrl: '' }); }} className="mt-3 min-h-12 w-full bg-[#15333b] font-black text-white disabled:bg-slate-300 rounded-md">Добавить точку прогресса</button><div className="mt-4 grid gap-2">{[...(state.bodyLogs || [])].reverse().map((item) => <article key={item.id} className="flex items-center gap-3 border border-[#d7e5e1] p-3 rounded-md">{item.photoUrl && <img src={item.photoUrl} alt="Прогресс" className="h-14 w-14 object-cover rounded-md" />}<div className="min-w-0"><strong>{formatLongDate(item.date)}</strong><div className="mt-1 text-xs font-bold text-slate-500">{item.weight && `${item.weight} кг · `}{item.waist && `талия ${item.waist} см · `}{item.chest && `грудь ${item.chest} см`}</div></div></article>)}</div></section></div>;
}
...
function numberOr(value, fallback) { const parsed = Number(String(value ?? '').replace(',', '.')); return Number.isFinite(parsed) ? parsed : fallback; }

function Confirm({ title, text, onConfirm, onClose }) { return <div role="dialog" aria-modal="true" aria-label={title} className="fixed inset-0 z-50 grid place-items-center bg-[#15333b]/50 p-3"><section className="w-full max-w-md bg-white p-5 shadow-2xl rounded-lg"><div className="flex items-start justify-between gap-3"><h2 className="text-2xl font-black">{title}</h2><button type="button" onClick={onClose} className="icon-command"><X size={18} /></button></div><p className="mt-3 font-semibold leading-7 text-slate-600">{text}</p><div className="mt-5 grid grid-cols-2 gap-2"><button type="button" onClick={onClose} className="min-h-12 border border-[#cfe0dc] font-black rounded-md">Отмена</button><button type="button" onClick={onConfirm} className="min-h-12 bg-[#0d8b71] font-black text-white rounded-md">Да, закрыть</button></div></section></div>; }
