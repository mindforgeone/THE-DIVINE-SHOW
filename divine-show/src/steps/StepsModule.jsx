  ...
  return (
    <div className="steps-module grid gap-4">
      ...
      <AnimatePresence>
        ...
        {scheduleStep && <ScheduleModal target={scheduleStep} today={today} onSchedule={schedule} onClose={() => setScheduleStep(null)} />}
        ...
        {historyStep && <HistoryModal step={historyStep} state={state} onEdit={setEditExecution} onRepeat={() => { setHistoryStep(null); setFinishStep(historyStep); }} onClose={() => setHistoryStep(null)} />}
        ...
        {categoryEditor && <CategoryEditor state={state} commit={commit} onDelete={(category) => confirmDelete('category', category)} onClose={() => setCategoryEditor(false)} />}
        {confirm && <Dialog title="Удалить запись?" onClose={() => setConfirm(null)}><p className="text-sm leading-6 text-slate-600">История останется защищённой от случайного нажатия. Статистика будет пересчитана по оставшимся записям.</p><div className="mt-5 flex gap-2"><button type="button" onClick={() => setConfirm(null)} className={`${BUTTON} flex-1 border border-[#d8e3e7]`}>Отмена</button><button type="button" onClick={deleteConfirmed} className={`${BUTTON} flex-1 bg-[#b84b52] text-white`}>Удалить</button></div></Dialog>}
      </AnimatePresence>
