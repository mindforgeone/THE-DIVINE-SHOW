...
export default function StepsModule({ state, commit, syncState, error, retry, goals = [], lifeState, newStepRequest = 0 }) {
  ...
  return (
    <div className="steps-module grid gap-4">
      ...
      <AnimatePresence>
        ...
        {taskEditor && <TaskEditor key={taskEditor.id || 'new'} task={taskEditor} categories={state.categories.filter(activeItem)} goals={goals} wishes={lifeState?.wishes?.filter((item) => !item.deletedAt) || []} skills={lifeState?.skills?.filter((item) => !item.deletedAt) || []} onSave={saveTask} onDelete={(task) => confirmDelete('energyTasks', task)} onClose={() => setTaskEditor(null)} />}
...
function TaskEditor({ task, categories, goals, wishes, skills, onSave, onDelete, onClose }) {
  const [draft, setDraft] = useState({ title: '', description: '', categoryId: 'life', estimatedMinutes: '', estimatedCost: '', actualMinutes: '', actualCost: '', annoyance: 3, status: 'hanging', dueDate: '', scary: false, goalId: '', wishId: '', skillId: '', ...task });
  return <Dialog title={task.id ? 'Изменить дело' : 'Новое дело'} onClose={onClose}><form onSubmit={(event) => { event.preventDefault(); onSave(draft); }} className="grid gap-3"><label className="form-label">Что нужно сделать?<input required maxLength={120} value={draft.title} onChange={(event) => setDraft({ ...draft, title: event.target.value })} className="field-control mt-1" /></label><label className="form-label">Описание<textarea rows={2} maxLength={500} value={draft.description} onChange={(event) => setDraft({ ...draft, description: event.target.value })} className="field-control mt-1 resize-none" /></label><div className="grid grid-cols-2 gap-2"><label className="form-label">Категория<select value={draft.categoryId} onChange={(event) => setDraft({ ...draft, categoryId: event.target.value })} className="field-control mt-1">{categories.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="form-label">Дата<input type="date" value={draft.dueDate || ''} onChange={(event) => setDraft({ ...draft, dueDate: event.target.value })} className="field-control mt-1" /></label></div><div className="grid gap-2 sm:grid-cols-3"><label className="form-label">Цель<select value={draft.goalId || ''} onChange={(event) => setDraft({ ...draft, goalId: event.target.value })} className="field-control mt-1"><option value="">Без связи</option>{goals.filter((item) => item.active !== false).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="form-label">Желание<select value={draft.wishId || ''} onChange={(event) => setDraft({ ...draft, wishId: event.target.value })} className="field-control mt-1"><option value="">Без связи</option>{wishes.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label><label className="form-label">Навык<select value={draft.skillId || ''} onChange={(event) => setDraft({ ...draft, skillId: event.target.value })} className="field-control mt-1"><option value="">Без связи</option>{skills.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label></div><div className="grid grid-cols-2 gap-2"><label className="form-label">Примерно минут<input type="number" min="0" value={draft.estimatedMinutes} onChange={(event) => setDraft({ ...draft, estimatedMinutes: event.target.value })} className="field-control mt-1" /></label><label className="form-label">Примерно ₽<input type="number" min="0" value={draft.estimatedCost} onChange={(event) => setDraft({ ...draft, estimatedCost: event.target.value })} className="field-control mt-1" /></label></div>{task.status === 'done' && <div className="grid grid-cols-2 gap-2"><label className="form-label">По факту минут<input type="number" min="0" value={draft.actualMinutes ?? ''} onChange={(event) => setDraft({ ...draft, actualMinutes: event.target.value })} className="field-control mt-1" /></label><label className="form-label">По факту ₽<input type="number" min="0" value={draft.actualCost ?? ''} onChange={(event) => setDraft({ ...draft, actualCost: event.target.value })} className="field-control mt-1" /></label></div>}<label className="form-label">Сложность / напряжение: {draft.annoyance} из 5<input type="range" min="1" max="5" value={draft.annoyance} onChange={(event) => setDraft({ ...draft, annoyance: event.target.value })} className="mt-2 w-full" /></label><label className="flex items-center gap-3 border border-[#d8e3e7] bg-[#f7f9fa] p-3 text-sm font-black rounded-md"><input type="checkbox" checked={Boolean(draft.scary)} onChange={(event) => setDraft({ ...draft, scary: event.target.checked })} className="h-5 w-5 accent-[#df764a]" />Это страшный шаг</label><button type="submit" className={`${BUTTON} min-h-12 bg-[#16a36a] text-white`}>Сохранить дело</button></form>{task.id && <button type="button" onClick={() => onDelete(task)} className={`${BUTTON} mt-3 text-[#b84b52]`}><Trash2 size={16} />Удалить</button>}</Dialog>;
}
...
function UnloadView({ state, stats, onAdd, onEdit, onMove, onDelete }) {
  ...
  return <><div className="flex flex-wrap items-center justify-between gap-2"><div><h2 className="text-xl font-black">Общий backlog</h2><p className="text-sm font-semibold text-slate-500">Перетаскивай дела между этапами или меняй статус в карточке.</p></div><button type="button" onClick={onAdd} className={`${BUTTON} bg-[#16a36a] text-white`}><Plus size={18} />Добавить дело</button></div><section className="steps-band"><div className="flex items-center gap-2 text-sm font-black text-[#16865f]"><CheckCircle2 size={18} />Снято с головы · {stats.period === 'week' ? 'эта неделя' : PERIODS.find(([id]) => id === stats.period)?.[1].toLowerCase()}</div><div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">{[['Дел', stats.energy.count], ['Времени', `${Math.floor(stats.energy.minutes / 60)} ч ${stats.energy.minutes % 60} мин`], ['Потрачено', `${stats.energy.cost.toLocaleString('ru-RU')} ₽`], ['Источников напряжения', stats.energy.count]].map(([label, value]) => <div key={label} className="border-l-2 border-[#16a36a] bg-white p-2"><span className="block text-xs font-bold text-slate-500">{label}</span><strong className="text-base">{value}</strong></div>)}</div></section><div className="steps-kanban grid gap-2 md:grid-cols-2 xl:grid-cols-4">{ENERGY_STATUSES.filter((status) => status.id !== 'done').map((status) => <section key={status.id} className="min-h-36 border border-[#d8e3e7] bg-[#f7fafb] p-3 rounded-md" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const id = event.dataTransfer.getData('text/plain'); if (id) onMove(id, status.id); }}><div className="mb-3 flex justify-between text-sm font-black"><h3>{status.label}</h3><span className="text-slate-400">{tasks.filter((task) => task.status === status.id).length}</span></div><div className="grid gap-2">{tasks.filter((task) => task.status === status.id).map((task) => <TaskCard key={task.id} task={task} state={state} onEdit={onEdit} onMove={onMove} onDelete={onDelete} />)}</div></section>)}</div>{archived.length > 0 && <section className="steps-band"><button type="button" onClick={() => setShowDone((value) => !value)} aria-expanded={showDone} className="flex w-full items-center justify-between text-left font-black"><span>Готово · {archived.length}</span><ChevronDown size={18} className={showDone ? 'rotate-180' : ''} /></button>{showDone && <div className="mt-3 grid gap-2 sm:grid-cols-2">{archived.map((task) => <TaskCard key={task.id} task={task} state={state} onEdit={onEdit} onMove={onMove} onDelete={onDelete} />)}</div>}</section>}</>;
}

function TaskCard({ task, state, onEdit, onMove, onDelete }) {
  return <article draggable onDragStart={(event) => event.dataTransfer.setData('text/plain', task.id)} className="steps-card border border-[#d8e3e7] bg-white p-3 rounded-md"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><div className="text-xs font-bold text-[#0d8fb9]">{categoryName(state, task.categoryId)} · {task.annoyance}/5{task.scary ? ' · страшный шаг' : ''}</div><h4 className="mt-1 break-words text-sm font-black">{task.title}</h4></div><button type="button" onClick={() => onEdit(task)} className="icon-command h-8 w-8 shrink-0" title="Редактировать дело"><Edit3 size={15} /></button></div>{task.description && <p className="mt-2 text-xs leading-5 text-slate-500">{task.description}</p>}<div className="mt-2 flex flex-wrap gap-3 text-xs font-semibold text-slate-500"><span>{task.estimatedMinutes || 0} мин</span><span>{Number(task.estimatedCost || 0).toLocaleString('ru-RU')} ₽</span><span>{task.dueDate ? `до ${formatShortDate(task.dueDate)}` : formatShortDate(task.createdAt.slice(0, 10))}</span></div>{(task.goalId || task.wishId || task.skillId) && <div className="mt-2 text-[11px] font-black text-[#6751bb]">Связано: {[task.goalId && 'цель', task.wishId && 'желание', task.skillId && 'навык'].filter(Boolean).join(', ')}</div>}<div className="mt-3 flex items-center gap-2"><select aria-label={`Статус: ${task.title}`} value={task.status} onChange={(event) => onMove(task.id, event.target.value)} className="field-control min-h-10 py-1 text-xs">{ENERGY_STATUSES.map((status) => <option key={status.id} value={status.id}>{status.label}</option>)}</select>{task.status !== 'done' ? <button type="button" onClick={() => onMove(task.id, 'done')} className="icon-command h-10 w-10 shrink-0 bg-[#eaf8f4] text-[#16865f]" title="Готово"><Check size={17} /></button> : <button type="button" onClick={() => onDelete('energyTasks', task)} className="icon-command h-10 w-10 shrink-0" title="Удалить"><Trash2 size={16} /></button>}</div></article>;
}
