  ...
  return (
    ...
    >
      <motion.section initial={{ y: 28 }} animate={{ y: 0 }} exit={{ y: 28 }} className="max-h-[92vh] w-full max-w-2xl overflow-y-auto bg-white p-4 shadow-2xl rounded-t-lg sm:rounded-lg sm:p-5">
        ...
        {rules.length > 0 && <section className="mt-4"><h3 className="font-black">Мой Кодекс</h3><div className="mt-2 grid gap-2">{rules.map(({ rule, value, answered, passed }) => <div key={rule.id} className="flex items-center gap-3 border border-[#dfe8e6] p-3 rounded-md"><span className={`grid h-8 w-8 shrink-0 place-items-center rounded-md ${!answered ? 'bg-slate-100 text-slate-400' : passed ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-600'}`}>{answered ? passed ? <Check size={17} /> : <X size={17} /> : <CircleDashed size={17} />}</span><span className="min-w-0 flex-1"><strong className="block text-sm">{rule.title}</strong><small className="font-bold text-slate-500">{!answered ? 'Нет отметки' : rule.type === 'boolean' ? passed ? 'Соблюдено' : 'Не выполнено' : `${value}${rule.unit ? ` ${rule.unit}` : ''}`}</small></span></div>)}</div></section>}
