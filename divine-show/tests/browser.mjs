...
try {
  ...
  assert.equal(await page.getByRole('heading', { name: 'Фотосъёмки' }).count(), 0, 'Optional goals must not crowd the day');
  await page.getByTitle('Выбрать цели на день').click();
  const goalPicker = page.getByRole('dialog', { name: 'Цели на этот день' });
  await goalPicker.getByRole('button', { name: 'Фотосъёмки' }).click();
  await goalPicker.getByRole('button', { name: 'Готово' }).click();
  await page.getByRole('heading', { name: 'Фотосъёмки' }).waitFor();
  await page.getByTitle('Редактировать цель Фотосъёмки').click();
  const goalDialog = page.getByRole('dialog', { name: 'Редактировать цель' });
  await goalDialog.getByRole('textbox', { name: 'Название' }).fill('Фото практика');
  await goalDialog.getByRole('button', { name: 'Сохранить цель' }).click();
  await page.getByRole('heading', { name: 'Фото практика' }).waitFor();
  await page.getByTitle('Убрать из этого дня: Фото практика').click();
  assert.equal(await page.getByRole('heading', { name: 'Фото практика' }).count(), 0);
  ...
  assert.equal(await page.getByRole('heading', { name: 'Фото практика' }).count(), 0, 'Day goal selection must survive reload');
  offline = false;
